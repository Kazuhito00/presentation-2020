...
def _process_input(source_img, scale_factor=1.0, output_stride=16):
    ...
    input_img = cv2.resize(
        source_img, (target_width, target_height),
...
def get_adjacent_keypoints(keypoint_scores,
    ...
    for left, right in posenet.CONNECTED_PART_INDICES:
        if keypoint_scores[left] < min_confidence or keypoint_scores[
                right] < min_confidence:
            continue
        results.append(
            np.array(
